...
terrain <- terrain[,c("EpPlotID","type","rw","hw","elevation","slope","aspect","Exploratorium")]
...
dataset_clean <- dataset[,-which(names(dataset)%in%c("datetime","timestamp","EpPlotID","SM_10_2","SM_15",
                                                     "SM_20","SM_20_2","SM_30","SM_35","SM_40","SM_50","type","Ts_20","Ts_5",
                                                     "Ts_50","rH_200","Std_procedure.exploratory."))]
#################################################################################
...
for (i in 1:nrow(dataset_clean)){
  if (is.na(dataset_clean$P_RT_NRT[i])|is.na(dataset_clean$Ta_200[i])){
    ...
    if (nrow(statNotNa)>0){
      ...
      if(substr(clim_near,1,1)!=substr(dataset_clean$plotID[i],1,1)){next} #if no data from same area
