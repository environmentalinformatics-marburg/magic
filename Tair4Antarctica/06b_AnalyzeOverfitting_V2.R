...
complete_df$method<-factor(complete_df$method,levels=c("LOSOCV","RSS"))
...
fullModel_cv_ptxt<-paste0("Rsq = ",sprintf("%.2f", round(fullModel_cv_regstat$R2,2)),
...
ffs_cv_ptxt<-paste0("Rsq = ",sprintf("%.2f", round(ffs_cv_regstat$R2,2)),
                    "\nRMSE = ",sprintf("%.2f", round(ffs_cv_regstat$RMSE,2)))

fullModel_rss_ptxt<-paste0("Rsq = ",sprintf("%.2f", round(mean(stat_full$Rsquared),2)),
       "\nRMSE = ",sprintf("%.2f", round(mean(stat_full$RMSE,2))))
ffs_rss_ptxt<-paste0("Rsq = ",sprintf("%.2f", round(mean(stat_ffs$Rsquared),2)),
                           "\nRMSE = ",sprintf("%.2f", round(mean(stat_ffs$RMSE,2))))
labs <- c(ffs_cv_ptxt,ffs_rss_ptxt,
          fullModel_cv_ptxt,fullModel_rss_ptxt)
