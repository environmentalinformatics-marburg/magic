cellHarmonics <- function(st, 
  ...
  # Extract values
  st.mat <- raster::getValues(st)
  nd.mat <- raster::getValues(nd)
  ...
  param.rst <- foreach(h = seq(2)) %do% {
    foreach(i = seq(4), .combine = "stack", .packages = lib, 
            .export = ls(envir = globalenv())) %dopar% {
  ...
  if (!is.null(path.out)) {
    param.rst <- foreach(i = param.rst, j = list("st", "nd"), .packages = lib, 
                         .export = ls(envir = globalenv())) %dopar% {
      names(i) <- paste(j, c("max_x", "max_y", "min_x", "min_y"), sep = "_")
