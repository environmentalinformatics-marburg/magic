...
for (data in dat.list){
  ...
  ##################### Ta_200 #####################################################################
  dat_t <- data.frame(Monat = month.abb, 
  ...
  p1 <- (ggplot(aes(x = Monat, y = Temperatur, group = Leg, label = Temperatur, 
           ...
           xlab(NULL)+
                 ...
                 geom_line() + 
                 geom_point() + 
                 geom_text(vjust = -0.5,hjust=-0.3, size=2, color="black") + 
                 ggtitle(paste( station ,var_Name_t, "°C ", year)) +
  ...
  ##################### rH_200 #####################################################################
  dat_r <- data.frame(Monat = month.abb, 
  ...
  p2 <- (ggplot(aes(x = Monat, y = Luftfeuchte, group = Leg, label = Luftfeuchte, 
           ...
           xlab(NULL)+
                 ...
                 geom_point() + 
                 geom_text(vjust = -0.5,hjust=-0.3, size=2, color="black") + 
                 ggtitle(paste( station ,var_Name_r, "in %", year)) +
           theme(plot.title=element_text(family="Times", face="bold", size=10)) +
  ...
  ##################### P_RT_NRT #####################################################################
  dat_w <- data.frame(Monat = month.abb, 
                      data[ c( parameter_w)])
  dat_w.mlt_w <- melt(dat_w)
  dat_w.mlt_w$Monat <- factor(dat_w.mlt_w$Monat, levels = month.abb)
  p<- colnames(dat_w.mlt_w)<-c('Monat','Leg',var_Name_w)
  
  p3 <- ggplot(dat_w.mlt_w , aes( y=Niederschlag, x=Monat, fill=Niederschlag))
  p3 <- p3 + geom_bar(position="dodge", stat="identity") + scale_size_area()+
        xlab(NULL)+
        
        ggtitle(paste(station, var_Name_w, "in mm", year)) + 
        theme(plot.title=element_text(family="Times", face="bold", size=10))+
        theme(legend.position="none") +
        geom_text(aes(y=Niederschlag, label = Niederschlag),vjust = 2,size=2)+
        theme(plot.margin = unit(c(0,8,8,8), "mm"))+
        theme(title = element_text(vjust=12))  +
