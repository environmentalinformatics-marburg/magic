function (stationpath, MSG_extract, UTC = 0) 
{
  matchfun <- function(i, MSG_extract, stationpath, files, 
                       filenames, UTC) {
    rainfall <- data.frame()
    ...
    statdat$datetime <- gsub(":", "", statdat$datetime)
    statdat$datetime <- as.POSIXct(statdat$datetime, format = "%Y%m%d%H%M") + 
      3600 * (-UTC)
    ...
    for (k in 1:nrow(statdatsub)) {
      if (any(statdatsub$date[k] == statdat$datetime) == 
          FALSE) {
        next
      }
 #     if (sum(is.na(data.frame(statdatsub[k, ], statdat[statdatsub$date[k] == 
#                                                        statdat$datetime, 4])[3:13])) == 11) {
#        next
#      }
      rainfall <- rbind(rainfall, data.frame(statdatsub[k, 
                                                        ], statdat[statdatsub$date[k] == statdat$datetime, 
                                                                   4]))
    }
  ...
  rslt <- foreach(i = 1:length(unique(MSG_extract$Station)), 
                  .errorhandling = "remove", .packages = lib, .combine = rbind) %dopar% 
                  {
