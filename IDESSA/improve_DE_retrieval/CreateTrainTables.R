...
if (datasetTime=="day"){
  ...
  derivVariables=c("T0.6_1.6","T6.2_10.8","T7.3_12.0","T8.7_10.8","T10.8_12.0","T3.9_7.3","T3.9_10.8","sunzenith")
...
if (datasetTime=="inb"){
  ...
  derivVariables=c("T6.2_10.8","T7.3_12.0","T8.7_10.8","T10.8_12.0","T3.9_7.3","T3.9_10.8","sunzenith")
...
### Meikes predictor variables #################################################
      ...
      scenerasters<-stack(scenerasters,T0.6_1.6,T6.2_10.8,T7.3_12.0,T8.7_10.8,T10.8_12.0,T3.9_7.3,T3.9_10.8,sunzenith)
      names(scenerasters)=c("VIS0.6","VIS0.8","NIR1.6","IR3.9","WV6.2","WV7.3","IR8.7",
                            "IR9.7","IR10.8","IR12.0","IR13.4","T0.6_1.6","T6.2_10.8","T7.3_12.0",
                            "T8.7_10.8","T10.8_12.0","T3.9_7.3","T3.9_10.8","sunzenith")
      scenerasters<-scenerasters[[c(variables,derivVariables)]]
      names(scenerasters)<- c(variables,derivVariables)
    
### Texture parameters #########################################################
      glcm_filter <- texture.variables (x=scenerasters[[xderivTexture]],n_grey = 32,
...
### zonal stat #################################################################
      tmpStats=zonal(scenerasters[[1:(nlayers(scenerasters)-1)]],cloud_geometry$cloudPatches,fun="mean")
      ZonalStats=cloud_geometry$cloudPatches
      MeanPerPatch=foreach(i=2:ncol(tmpStats),.combine=stack,.packages=c("raster","doParallel"))%dopar%{reclassify(ZonalStats,tmpStats[,c(1,i)])} 
      names(MeanPerPatch)=paste0("mean_",names(scenerasters)[1:(nlayers(scenerasters)-1)])

      tmpStats=zonal(scenerasters[[1:(nlayers(scenerasters)-1)]],cloud_geometry$cloudPatches,fun="sd")
      SdPerPatch=foreach(i=2:ncol(tmpStats),.combine=stack,.packages=c("raster","doParallel"))%dopar%{reclassify(ZonalStats,tmpStats[,c(1,i)])} 
      names(SdPerPatch)=paste0("sd_",names(scenerasters)[1:(nlayers(scenerasters)-1)])
...
#      reference<-eval(parse(text=paste0("scenerasters$",referenceimage)))
      ...
      rm(scenerasters,cloud_geometry,glcm_3,glcm_5,glcm_filter,T6.2_10.8,T7.3_12.0,
         ...
         radarpathtmp,tmp,date,reference,MeanPerPatch,SdPerPatch,tmpStats,ZonalStats)
      gc()      
