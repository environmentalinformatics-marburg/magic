...
for (datasetTime in datasetTimes){
  for (responseName in responseNames){
    ...
    if (pc=="183"){
      ...
      rasterpathout <- paste0("/media/memory01/casestudies/hmeyer/Improve_DE_retrieval/results/predictions/",
                              datasetTime,"_",responseName)
      if(onlySpectral){
        rasterpathout <- paste0("/media/memory01/casestudies/hmeyer/Improve_DE_retrieval/results/predictions/onlySpectral",
                                datasetTime,"_",responseName)
      }
    }
    dir.create(rasterpathout)
    type="classification"
    if(responseName=="Rain"){type="regression"}
    ################################################################################
    if(!onlySpectral){
      load(paste0(resultpath,"/trainedModels/trainedModel_",datasetTime,"_",responseName,".Rdata"))
    }
    if(onlySpectral){
      load(paste0(resultpath,"/trainedModels/trainedModel_OnlySpec_",datasetTime,"_",responseName,".Rdata"))
    }
    ...
    if (type=="classification"){
      evaluation_validation=data.frame(matrix(ncol=9))
      evaluation_training=data.frame(matrix(ncol=9))
      names(evaluation_validation)=c("scene","Bias", "PFD", "FAR", "POD", "CSI", "ETS", "HSS", "HKD")
      names(evaluation_training)=c("scene","Bias", "PFD", "FAR", "POD", "CSI", "ETS", "HSS", "HKD")
    }
    ...
    for (i in 1:nrow(rainevents)){
      ...
      writeRaster(reference, filename=paste0(rasterpathout,"/ref_",
                                        responseName,"2010",month,
                                        day,time,".tif"), 
                  format='GTiff', overwrite=TRUE)
      #for training scenes:
    ...
    if(onlySpectral){
      save(evaluation_validation,file=paste0(resultpath,"validation/OnlySpectral_evaluation_validation_",datasetTime,
                                             "_",responseName,".Rdata"))
      save(evaluation_training,file=paste0(resultpath,"/validation/OnlySpectral_evaluation_training_",datasetTime,
                                           "_",responseName,".Rdata"))
      save(comp_training,file=paste0(resultpath,"/validation/OnlySpectral_globalComp_training.RData"))
      save(comp_validation,file=paste0(resultpath,"validation/OnlySpectral_globalComp_validation.RData"))
    }
  }
}
