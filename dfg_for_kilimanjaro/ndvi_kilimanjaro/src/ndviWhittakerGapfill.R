...
rst.wht <- whittaker.raster(ndvi.rst.qa.sd.fc, removeOutlier = TRUE, threshold = .2,
...
#                         pattern = "fullPeriod.tif$", full.names = TRUE)
...
rst_wht_aggmax <- 
  stackApply(rst.wht[[1]], indices = indices_agg, fun = max, bylayer = TRUE,
...
fls_scl <- paste0(outdir, "/SCL_", basename(fls_wht_aggmax))
