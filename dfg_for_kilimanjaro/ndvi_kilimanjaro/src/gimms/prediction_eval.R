...
mod_observed <- mod_stck_eval <- rst_modis_myd13[[-pred_ind]] # MODIS [[61:120]]
...
dir_out <- "data/rst/dwnscl_agg1km"
file_out <- paste0(dir_out, "/gimms_ndvi3g_dwnscl_0812_noreduceboth")
...
ls_scores <- foreach(i = paste("0812", c("noreduceboth", "reduceboth", "dsn_noreduceboth", "dsn_reduceboth"), sep = "_"), 
                     j = c("REDUCE.BOTH = FALSE, DESEASON = FALSE", "REDUCE.BOTH = TRUE, DESEASON = FALSE", 
                           "REDUCE.BOTH = FALSE, DESEASON = TRUE", "REDUCE.BOTH = TRUE, DESEASON = TRUE")) %do% {
  tmp_fls <- list.files("data/rst/dwnscl_agg1km", pattern = i, full.names = TRUE)
  tmp_rst <- raster(tmp_fls)
  
  tmp_val_obs <- getValues(mod_observed)
...
mlt_scores <- melt(df_scores, id.vars = "type")
...
foreach(i = rst_prd, j = as.list(fls_prd), 
  ...
  ## lm -> slope
  mod_obs_sl <- calc(if (DESEASON) mod_observed else mod_observed, fun = function(x) {
    model <- lm(x ~ seq(x))
...
#                        col.regions = rev(cols_div(100)), at = seq(-.006, .006, .001))
  
  mod_prd_sl <- calc(if (DESEASON) mod_predicted else mod_predicted, fun = function(x) {
    model <- lm(x ~ seq(x))
    p <- summary(model)$coefficients[2, 4]
    s <- summary(model)$coefficients[2, 1]
    s[p >= .001 | p <= -.001] <- NA
...
#     latticeCombineGrid(list(p_sl_obs, p_sl_prd), layout = c(1, 2))
  ...
  mod_obs_mk <- calc(if (DESEASON) mod_observed else mod_observed, fun = function(x) {
    mk <- MannKendall(x)
...
#   val_obs_mk <- getValues(mod_obs_mk)
  ...
  mod_prd_mk <- calc(if (DESEASON) mod_predicted else mod_predicted, fun = function(x) {
    mk <- MannKendall(x)
    if (mk$sl < .001) return(mk$tau) else return(NA)
...
#     p_mk <- latticeCombineGrid(list(p_mk_obs, p_mk_prd), layout = c(1, 2))
  ...
  png(paste0("vis/comparison_plots_agg1km_", basename(fls_out), ".png"), width = 22, 
