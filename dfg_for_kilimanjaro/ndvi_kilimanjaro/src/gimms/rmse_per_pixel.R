...
# path: input
ch_dir_eval <- "data/eot_eval_agg1km/"
...
p_slp_gt_rmse <- 
  spplot(rst_slp_gt_rmse, col.regions = cols_div(100), 
         main = list(label = "Regression slope", fontface = 1, fontsize = 15, cex = 1.2),
         at = seq(-.25, .25, .05), scales = list(draw = TRUE), 
         par.settings = list(fontsize = list(text = 15)),
         sp.layout = list(list("sp.lines", np_old_utm_sl, lwd = 1.6, lty = 2), 
                          list("sp.lines", np_new_utm_sl, lwd = 1.6))) + 
  as.layer(p_dem)

p_slp_gt_rmse_envin <- envinmrRasterPlot(p_slp_gt_rmse)
...
p_mk <- 
  spplot(mod_predicted_mk[[1]], col.regions = brewer.pal(10, "BrBG"), ylab = "y", 
...
png("vis/eval/slp_mk001.png", width = 26, height = 40, units = "cm", 
    res = 300, pointsize = 15)

# viewport for visualization of study area
vp0 <- viewport(x = 0, y = 1, height = .9, width = 1, 
...
vp1 <- viewport(x = 0.5, y = -0.035,
                height = 0.07, width = 1,
                just = c("centre", "top"),
                name = "key.vp")
pushViewport(vp1)

draw.colorkey(key = list(col = cols_div, width = 1, height = .75,
                         at = seq(-.5, .5, .1), 
                         space = "bottom"), draw = TRUE)
grid.text(expression("Kendall\'s" ~ tau), x = 0.5, y = 0.1, 
          just = c("centre", "top"), gp = gpar(fontface = 1, fontsize = 15, cex = 1.2))
