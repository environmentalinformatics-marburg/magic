"""Concat Idrisi datasets which are unprocessed Terra-ASTER datasets.
...
def main():
    ...
    '''
    print
    print 'Module: concatIdrisiAster'
    ...
    for dataset in satellite_datasets:
        ...
        occurence = list(find_strings(\
            os.path.splitext(os.path.basename(dataset))[0], "_"))[-2]
        temp_filepath = output_path + os.sep + \
            os.path.splitext(os.path.basename(dataset))[0][0:3] + \
            os.path.splitext(os.path.basename(dataset))[0][occurence:] + \
            "_Stemp" + os.path.splitext(os.path.basename(dataset))[1]
        idrisi_cmd = dataset + "*" + temp_filepath
        ###IDRISI32.RunModule('STANDARD', idrisi_cmd, 1, '', '', '', '', 1)
        
        #Apply mask to set image background to 0
        mask_filepath = output_path + os.sep + \
            os.path.splitext(os.path.basename(dataset))[0][0:3] + \
            os.path.splitext(\
                os.path.basename(dataset))[0][occurence:] + \
            "_mask" + os.path.splitext(os.path.basename(dataset))[1]
        idrisi_cmd = "i*" + dataset + "*" + mask_filepath + \
            "*2*1*0.0001*999999*-9999*1"
        ###IDRISI32.RunModule('RECLASS', idrisi_cmd, 1, '', '', '', '', 1)
        output_filepath = output_path + os.sep + \
            os.path.splitext(os.path.basename(dataset))[0][0:3] + \
            os.path.splitext(os.path.basename(dataset))[0][occurence:] + \
            "_S" + os.path.splitext(os.path.basename(dataset))[1]
        idrisi_cmd = "3*" + temp_filepath + "*" + mask_filepath + "*" + \
                output_filepath
        ###IDRISI32.RunModule('OVERLAY', idrisi_cmd, 1, '', '', '', '', 1)
        
        #Add file to concatenate group
        if os.path.splitext(os.path.basename(output_filepath))[0][-4:] in \
            concat_filepath:
            concat_filepath[os.path.splitext(os.path.basename(\
                output_filepath))[0][-4:]].append(output_filepath)
        else:
            concat_filepath[os.path.splitext(os.path.basename(\
                output_filepath))[0][-4:]] = [output_filepath]
    ...
    for entry in concat_filepath:
        ...
        output_path, output_file  = os.path.split(concat_filepath[entry][0])
        occurence = list(find_strings(output_file, "_"))[-2]
        output_filepath = output_path + os.sep + \
            output_file[0:3] + output_file[occurence:]
