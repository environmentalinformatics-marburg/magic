...
## Models
load("/home/lehnert/Öffentlich/Dropbox/Paper_Hanna/aktuelles/rfeModel_Biomass.RData")
...
for (i in 1:length(img_dir))
{
  ...
  if (sat[[1]] %in% c(4))
  {
    ...
    outfile <- file.path(outdir, "biomass", paste(basename(img_dir[i]), "_sat_", sat[[1]] - 1, ".tif", sep = "")) 
    ...
    for (irow in 1:tr$n) 
    {
      ...
      for (n in 1:length(rfeModel_Biomass[[sat[[1]]]]$optVariables))
      {
        if (sum(names(nri_vals) == rfeModel_Biomass[[4]]$optVariables[n]) != 1)
          stop()
      }
      ...
      pred_vals <- predict(rfeModel_Biomass[[sat[[1]]]], nri_vals)
